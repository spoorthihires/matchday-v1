...
export async function setDecision(employerId: string, driveId: string, jobseekerId: string, decision: 'Shortlisted' | 'Hold' | 'Rejected' | null) {
  ...
  if (decision === null) {
    ...
    // and `consent`/`offer` guard a row carrying granted consent or an offer, from
    // being deleted out from under the board.
    const { deletedCount } = await Application.deleteOne({ employerId, driveId, jobseekerId, notes: { $size: 0 }, consent: { $exists: false }, stage: null, offer: { $exists: false } });
